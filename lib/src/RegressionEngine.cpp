...
struct DiscardCoefficients {
    ...
    void add(VectorXd&& cur_coefficients) { coefficients = std::move(cur_coefficients);}
...
ExecutionStrategy find_strategy(
    ...
    auto result = ExecutionStrategy(option_value.rows(),
                                    option_value.cols() - 1);
    for (auto i = option_value.cols() - 2; i >= 0; --i) {
        // regress values at i+1 to regressands at i. Values at T are equal to payoff
        coefficients_storage.add(regressands[i].
            bdcSvd(ComputeThinU | ComputeThinV).solve(option_value.col(i + 1)));
        // get current option values estimated based on regression
        VectorXd regression_values = regressands[i] * coefficients_storage.current();
...
FullExecutionStrategy find_full_strategy(
    ...
    auto coefficients_policy = StoreCoefficients{result.regression_coefficients};
    result.executions = find_strategy(regressands, option_value, coefficients_policy);
    return result;
}
